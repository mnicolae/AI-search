...
#IMPLEMENT
        ...
        if get_loc(self) == 'home':
            ...
            for job_name in get_unstarted(self):
                job_info = get_job_info(get_job_list(self), job_name)
                ...
                if get_load(self) + job_weight <= get_max_weight(self):
        ...
        # Location is not home, can only perform pickup or deliver actions
                    ...
                    for carried_job_name in self.state['jobs_carried']:
                        ...
                        if cannot_pickup == False:
                            new_state = {"jobs_carried": list(get_carrying(self)).append(job_name), \
                                        ...
                                        "time": job_pickup_time, \
                                        "earnings": get_earned(self), \
                                        "jobs_not_started": list(get_unstarted(self)).remove(job_name)}
            ...
            for job_name in get_carrying(self):
                ...
                travel_time = get_travel_time(self.state['location'], job_destination_location)
                new_state_time = get_time(self) + travel_time
                job_earning = get_job_earning(job_name, new_state_time)
                
                if new_state_time >= get_max_deliver(self):
                    new_state = {"jobs_carried": list(get_carrying(self)).remove(job_name), \
                                "jobs_carried_weight": get_load(self) - job_weight, \
                                "location": job_destination_location, \
                                "time": new_state_time, \
                                "earnings": job_earning, \
                                "jobs_not_started": list(get_unstarted(self))}
